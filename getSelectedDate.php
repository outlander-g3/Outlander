...
try{
    ...
    $sql = 'select b.pdStart,a.*, avg(rate) avgRate 
    from productkind a join product b on a.pdkNo=b.pdkNo
    join `order` c on b.pdNo=c.pdNo
    where continent is not null and pdStart > :pdStart group by pdkNo ';//為啥只取回一筆?
      ...
      "<div class='pro-item pro-item-three'>
            <a href='products.html'>
                ...
                <div class='pro-item-view-flex'>
                    <p>評價：</p>";
                    for($i=0; $i<floor($dateSearchRow['avgRate']); $i++){ 
                      $html .= '<img src="img/tree_j.png">';
                     } 
                      if($dateSearchRow['avgRate']*10%10 != 0){
                ...
                <div class='pro-item-view-float'>
                    ...
                    <div class='hike-float'>
                        <span class='hike'>";
                        for($i=0; $i<$dateSearchRow["level"]; $i++){
